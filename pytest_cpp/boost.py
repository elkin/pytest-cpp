...
class NewBoostTestFacade(object):
    """
    Facade for BoostTests.
    """

    _JUNIT_LOG_FORMAT = re.compile(r'--log_format=<(JUNIT|.*\|JUNIT\||.*\|JUNIT)>',
                                   re.MULTILINE)
    ...
    def is_test_suite(cls, executable):
        try:
            ...
            return '--output_format' in output \
                   and NewBoostTestFacade._JUNIT_LOG_FORMAT.search(output)
